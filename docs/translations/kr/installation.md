...
다음은 최신 Gaia 버전을 설치하는 것입니다. 필요에 따라 `git checkout`을 통해 올바른 [릴리즈 버전](https://github.com/JaTochNietDan/gaia/releases)이 설치되어있는지 확인하세요.

```bash
git clone -b <latest-release-tag> https://github.com/JaTochNietDan/gaia
...
```
# github.com/JaTochNietDan/gaia/cmd/gaiad
...
- 변경 사항 적용
- `go.mod`에 `replace github.com/JaTochNietDan/cosmos-sdk => /path/to/clone/cosmos-sdk` 추가
